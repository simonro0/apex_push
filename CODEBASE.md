...
| B5 | Burnout-Satz doppelt / leer          | 🔧 | Der letzte reguläre Satz entspricht funktional dem Burnout-Satz (maximale Wdh.). Wird danach noch ein separater Burnout-Satz gestartet und die Session sofort beendet, entsteht ein Split-Eintrag mit 0 Wdh. — sieht in der Strava-Beschreibung (`📊 Sets: … · 0`) und in der Share-Karte schlecht aus. **Fix:** Splits mit 0 Wdh. am Ende vor der Anzeige / dem Export herausfiltern **und** prüfen, ob der Burnout-Satz nach Abschluss aller regulären Sätze überhaupt noch gestartet wird — ggf. Training direkt nach dem letzten Pflicht-Satz als abgeschlossen markieren. Betroffene Stellen: `WorkoutProvider.recordSetSplit()`, `_description()` in `strava_service.dart`, `ShareCard`, `SessionDetailScreen`. |
| B6 | Play Store – Disclaimer fehlt        | 🔧 | Für die Veröffentlichung im Google Play Store wird ein Disclaimer / Datenschutz-Hinweis benötigt. Inhalte: (1) Keine Haftung für Zählgenauigkeit oder gesundheitliche Empfehlungen, (2) Verweis auf Open-Source-Repo (Apache 2.0), (3) Einladung zu Feedback und Pull Requests. Kann als In-App-Screen (`AboutScreen` oder eigener `DisclaimerScreen`) und als Store-Listing-Text umgesetzt werden. |
